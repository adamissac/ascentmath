...
function SubjectLevelCard({ tier, isBrand }: { tier: TutoringTier; isBrand: boolean }) {
  const cardClass = isBrand
    ...
    : "card group relative flex h-full min-w-[21rem] shrink-0 snap-start flex-col overflow-hidden p-6 transition-shadow duration-300 hover:shadow-[var(--shadow-card-hover)] lg:min-w-0";
...
function TierPopularRibbon() {
  return (
    <div className="tier-corner-ribbon" aria-label="Popular tier" title="Popular">
      <span className="tier-corner-ribbon__label">
        <svg
          viewBox="0 0 24 24"
          fill="currentColor"
          aria-hidden
          className="tier-corner-ribbon__star"
        >
