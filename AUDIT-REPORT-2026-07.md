...
| C1 | CRITICAL | `/book`, `/mathematics/unit-*` | Missing permanent redirects for legacy flat unit URLs; `/book` was 302 | Added 301 redirects for `unit-1`…`unit-7` → grade-6 units; `/book` and `/tutoring` now permanent | `feat(redirects): …` | `curl -sI` → 308 + Location |
...
| D1 | HIGH | unit pages | Worksheets only on topic pages | Added `UnitWorksheetsSection` rendering topic worksheets on unit overview | `feat(units): …` | build + component |
| D2 | HIGH | curriculum-frameworks | Meta claimed G6–8; only G6 data exists | Updated meta + on-page “G7 & 8 coming soon” note | `fix(frameworks): …` | copy review |
...
| F3 | MEDIUM | `/` credentials | “Algebra EOC · EOC” duplicate | Changed detail to “End-of-Course exam” | `fix(copy): …` | credentials data |
...
| §7 | — | data | No validation script | Added `scripts/audit/validate-content.ts` + `npm run audit:validate` | `feat(audit): …` | script output |
...
| Item | Value |
|------|-------|
| Ending HEAD | _(see latest commit on main)_ |
| Build | PASS |
